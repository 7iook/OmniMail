import { AlertCircle, LoaderCircle, MailPlus, Send, X } from 'lucide-react'
    ...
    <div className="compose-backdrop">
      ...
      >
        ...
        <div className="compose-dialog__body">
          <p id="compose-description">{t('通过 Resend 安全发送，并保存到已发送邮件。')}</p>
          <label>
            <span>{t('发件人')}</span>
          ...
          <label>
            <span>{t('收件人')}</span>
            <input name="to" type="email" autoComplete="off" spellCheck={false}
        ...
        <footer>
          <button className="button button--secondary" type="button"
            onClick={onClose} disabled={sending}>{t('取消')}</button>
