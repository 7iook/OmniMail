...
import {
  ...
  type PageInfo,
  type QqMailAccount,
  type QqMailMessageDetail,
  type QqMailMessageSummary,
} from '../../../shared/api'
...
import { QqMailAccountDialog } from './QqMailAccountDialog'
import { QqMailComposeDialog, type QqMailComposeInput } from './QqMailComposeDialog'
import { QqMailIcon } from './QqMailIcon'
import { QqMailReader } from './QqMailReader'
import { QqMailScopeSwitcher } from './QqMailScopeSwitcher'
import { QqMailSearchField } from './QqMailSearchField'
...
function accountError(accounts: QqMailAccount[]): string {
  const failed = accounts.filter(({ status }) => status === 'error' || status === 'credential_error')
  return failed.length ? t('{count} 个 QQ 邮箱账号需要处理，其他账号仍会继续同步。', {
    count: failed.length,
  }) : ''
}
...
export function QqMailWorkspace({ enabled, remoteImagesEnabled, canSend }: {
  enabled: boolean
  ...
  const [selected, setSelected] = useState<QqMailMessageSummary | null>(null)
  const [detail, setDetail] = useState<QqMailMessageDetail | null>(null)
  ...
  const loadAccounts = useCallback(async () => {
    ...
    const result = await api.qqMailAccounts()
  ...
  const loadMessages = useCallback(async (quiet = false) => {
    ...
    setError('')
    try {
      const result = await api.qqMailMessages(accountId, '', searchQuery, controller.signal)
  ...
  async function syncRemote() {
    ...
    setNotice('')
    try {
      const results = await Promise.allSettled(eligible.map(({ id }) => api.syncQqMail(id)))
      ...
      setNotice(t(accepted.length === 1
        ? 'QQ 邮箱同步任务已加入队列。'
        : '已加入 {count} 个 QQ 邮箱同步任务。', { count: accepted.length }))
      ...
      for (let attempt = 0; attempt < SYNC_POLL_ATTEMPTS; attempt += 1) {
        ...
        if (failed.length || rejected.length) {
          setError(t('部分 QQ 邮箱账号同步失败，请在账号管理中查看状态。'))
        } else {
          setNotice(t(accepted.length === 1
            ? 'QQ 邮箱同步完成。'
            : '已完成 {count} 个 QQ 邮箱账号同步。', { count: accepted.length }))
        }
        return
      }
      await loadMessages(true)
  ...
  async function selectMessage(message: QqMailMessageSummary) {
    messageController.current?.abort()
    const controller = new AbortController()
    ...
    setError('')
    try {
      const result = await api.qqMailMessage(message.account.id, message.id, controller.signal)
  ...
  async function loadMore() {
    ...
    try {
      const current = listRequestId.current
      const result = await api.qqMailMessages(accountId, page.nextCursor, searchQuery)
      if (current !== listRequestId.current) return
      setMessages((items) => {
  ...
  function openReply() {
    ...
    setCompose({
      ...
      initialSubject: /^re:/i.test(selected.subject)
        ? selected.subject : `Re: ${selected.subject || t('无主题')}`,
    })
  }

  async function sendMessage(input: QqMailComposeInput) {
    if (!compose) return
    setSending(true)
    setComposeError('')
    try {
      await api.sendQqMail(compose.account.id, {
        ...input,
        replyToMessageId: compose.replyToMessageId,
      })
      setCompose(null)
      setNotice(t('QQ 邮件已加入发送队列。'))
    } catch (sendError) {
  ...
  return <div className={`icloud-mail-view gmail-workspace gmail-mail-view${selected ? ' has-selection' : ''}`}>
    <section ref={mailListScroll.listPane}
      ...
      <header className="list-header icloud-list-header gmail-list-header">
        <div>{enabled && accounts.length ? <QqMailScopeSwitcher accounts={accounts}
          selectedAccountId={accountId} onChange={chooseAccount}
          onManage={() => setDialogMode('manage')} />
          : <p className="eyebrow">QQ MAIL · IMAP</p>}
          <ListScrollTopHeading title={t('QQ 邮箱')} onScrollTop={mailListScroll.scrollToTop} /></div>
        {enabled && <div className="list-header__actions">
          ...
          <div className="icloud-header-action-buttons">
            <button className="button button--primary compose-trigger" type="button"
              disabled={!canSend || !currentAccount || currentAccount.status === 'credential_error'}
              onClick={openNewMessage} aria-label={t('新建 QQ 邮件')}
              data-tooltip={t(!canSend ? '当前账户没有发信权限。'
                : currentAccount ? '新建 QQ 邮件' : '请先选择一个 QQ 邮箱账号')}>
            ...
            <button ref={manageButton} className="icon-button" type="button"
              onClick={() => setDialogMode('manage')} aria-label={t('管理 QQ 邮箱账号')}
              data-tooltip={t('管理 QQ 邮箱账号')}>
            ...
            <button className="icon-button" type="button" disabled={!accounts.length || remoteSyncing}
              ...
              aria-label={t(accountId ? '同步当前 QQ 邮箱账号' : '同步全部 QQ 邮箱账号')}
              data-tooltip={t(accountId ? '同步当前 QQ 邮箱账号' : '同步全部 QQ 邮箱账号')}>
              {remoteSyncing ? <LoaderCircle className="spin" size={17} /> : <RefreshCw size={17} />}
            </button>
          </div>
        </div>}
      </header>
      {enabled && accounts.length > 0 && <QqMailSearchField value={query} loading={loading}
        onChange={setQuery} />}
      ...
      <div className="gmail-message-list" aria-busy={loading}>
        {!enabled ? <div className="icloud-empty">
          ...
          <span><QqMailIcon width={25} height={25} aria-hidden="true" /></span>
          <h2>{t('连接你的第一个 QQ 邮箱')}</h2>
          <p>{t('使用 QQ 邮箱授权码，安全聚合最近的 INBOX 邮件。')}</p>
          <button className="button button--primary" type="button" onClick={() => setDialogMode('add')}>
            <Plus size={16} />{t('添加 QQ 邮箱账号')}</button>
          ...
          {messages.map((message) => {
            ...
            return <article className={`message-row${message.isRead ? '' : ' is-unread'}${active ? ' is-selected' : ''}`}
              ...
              <button className="message-row__main" type="button" onClick={() => void selectMessage(message)}>
                ...
                <span className="mailbox-hint"><QqMailIcon width={12} height={12}
    ...
    <main className="reader-pane icloud-reader-pane gmail-reader-pane">
      <QqMailReader selected={selected} message={detail} loading={detailLoading}
        error={detailError} remoteImagesEnabled={remoteImagesEnabled}
        onBack={() => { setSelected(null); setDetail(null); setDetailError('') }}
        onRetry={() => selected && void selectMessage(selected)} onReply={openReply} />
    </main>
