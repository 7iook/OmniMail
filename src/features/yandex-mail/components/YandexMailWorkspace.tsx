...
import {
  api,
  type PageInfo,
  type YandexMailAccount,
  type YandexMailMessageDetail,
  type YandexMailMessageSummary,
} from '../../../shared/api'
...
import { YandexMailAccountDialog } from './YandexMailAccountDialog'
import { YandexMailIcon } from './YandexMailIcon'
import { YandexMailReader } from './YandexMailReader'
import { YandexMailScopeSwitcher } from './YandexMailScopeSwitcher'
import { YandexMailSearchField } from './YandexMailSearchField'

const emptyPage: PageInfo = { hasMore: false, nextCursor: null, limit: 30 }
const SYNC_POLL_ATTEMPTS = 15
const ACCOUNT_DIALOG_POLL_MS = 1_000

function delay(milliseconds: number): Promise<void> {
  return new Promise((resolve) => window.setTimeout(resolve, milliseconds))
}

function accountError(accounts: YandexMailAccount[]): string {
  const failed = accounts.filter(({ status }) => status === 'error' || status === 'credential_error')
  return failed.length ? t('{count} 个 Yandex 邮箱账号需要处理，其他账号仍会继续同步。', {
    count: failed.length,
  }) : ''
}

export function YandexMailWorkspace({ enabled, remoteImagesEnabled }: {
  enabled: boolean
  ...
  const [selected, setSelected] = useState<YandexMailMessageSummary | null>(null)
  const [detail, setDetail] = useState<YandexMailMessageDetail | null>(null)
  ...
  const loadAccounts = useCallback(async () => {
    ...
    const result = await api.yandexMailAccounts()
  ...
  useEffect(() => {
    if (!enabled || !dialogMode) return
    let disposed = false
    let inFlight = false
    const pollAccounts = async () => {
      if (inFlight) return
      inFlight = true
      try { await loadAccounts() } catch { /* retain the last visible status */ }
      finally { inFlight = false }
    }
    void pollAccounts()
    const timer = window.setInterval(() => {
      if (!disposed) void pollAccounts()
    }, ACCOUNT_DIALOG_POLL_MS)
    return () => {
      disposed = true
  ...
  async function syncRemote() {
    ...
    if (!eligible.length) {
      setError(t(scoped.some(({ status }) => status === 'syncing')
        ? 'Yandex 邮箱同步正在进行，请稍候。'
        : '没有可同步的 Yandex 邮箱账号，请先更新失效的应用专用密码。'))
      return
    }
    ...
    setNotice('')
    try {
      const results = await Promise.allSettled(eligible.map(({ id }) => api.syncYandexMail(id)))
      ...
      setNotice(t(accepted.length === 1
        ? 'Yandex 邮箱同步任务已加入队列。'
        : '已加入 {count} 个 Yandex 邮箱同步任务。', { count: accepted.length }))
      ...
      for (let attempt = 0; attempt < SYNC_POLL_ATTEMPTS; attempt += 1) {
        ...
        if (failed.length || rejected.length) {
          setError(t('部分 Yandex 邮箱账号同步失败，请在账号管理中查看状态。'))
        } else {
          setNotice(t(accepted.length === 1
            ? 'Yandex 邮箱同步完成。'
            : '已完成 {count} 个 Yandex 邮箱账号同步。', { count: accepted.length }))
        }
        return
      }
      await loadMessages(true)
  ...
  async function loadMore() {
    ...
    try {
      const current = listRequestId.current
      const result = await api.yandexMailMessages(accountId, page.nextCursor, searchQuery)
      if (current !== listRequestId.current) return
      setMessages((items) => {
  ...
  return <div className={`icloud-mail-view gmail-workspace gmail-mail-view${selected ? ' has-selection' : ''}`}>
    <section ref={mailListScroll.listPane}
      ...
      <header className="list-header icloud-list-header gmail-list-header">
        <div>{enabled && accounts.length ? <YandexMailScopeSwitcher accounts={accounts}
          selectedAccountId={accountId} onChange={chooseAccount}
          onManage={() => setDialogMode('manage')} />
          : <p className="eyebrow">Yandex MAIL · IMAP</p>}
          <ListScrollTopHeading title={t('Yandex 邮箱')} onScrollTop={mailListScroll.scrollToTop} /></div>
        {enabled && <div className="list-header__actions">
          ...
          <div className="icloud-header-action-buttons">
            ...
            <button ref={manageButton} className="icon-button" type="button"
              onClick={() => setDialogMode('manage')} aria-label={t('管理 Yandex 邮箱账号')}
              data-tooltip={t('管理 Yandex 邮箱账号')}>
            ...
            <button className="icon-button" type="button" disabled={!accounts.length || remoteSyncing}
              ...
              aria-label={t(accountId ? '同步当前 Yandex 邮箱账号' : '同步全部 Yandex 邮箱账号')}
              data-tooltip={t(accountId ? '同步当前 Yandex 邮箱账号' : '同步全部 Yandex 邮箱账号')}>
              {remoteSyncing ? <LoaderCircle className="spin" size={17} /> : <RefreshCw size={17} />}
            </button>
          </div>
        </div>}
      </header>
      {enabled && accounts.length > 0 && <YandexMailSearchField value={query} loading={loading}
        onChange={setQuery} />}
      ...
      <div className="gmail-message-list" aria-busy={loading}>
        {!enabled ? <div className="icloud-empty">
          <span><KeyRound size={24} /></span><h3>{t('Yandex 邮箱功能尚未启用')}</h3>
          <p>{t('在 Worker Variables & Secrets 中配置至少 32 字节的 YANDEX_MAIL_CREDENTIALS_KEY，然后重新部署。')}</p>
        </div> : loading ? <div className="gmail-list-state" role="status">
          <LoaderCircle className="spin" size={21} />{t('正在读取 Yandex 邮箱索引…')}
        </div> : !accounts.length ? <div className="gmail-list-state gmail-list-state--empty">
          <span><YandexMailIcon width={25} height={25} aria-hidden="true" /></span>
          <h2>{t('连接你的第一个 Yandex 邮箱')}</h2>
          <p>{t('使用 Yandex 邮箱应用专用密码，安全聚合最近的 INBOX 邮件。')}</p>
          <button className="button button--primary" type="button" onClick={() => setDialogMode('add')}>
            <Plus size={16} />{t('添加 Yandex 邮箱账号')}</button>
          ...
          {messages.map((message) => {
            ...
            return <article className={`message-row${message.isRead ? '' : ' is-unread'}${active ? ' is-selected' : ''}`}
              ...
              <button className="message-row__main" type="button" onClick={() => void selectMessage(message)}>
                ...
                <span className="mailbox-hint"><YandexMailIcon width={12} height={12}
    ...
    <main className="reader-pane icloud-reader-pane gmail-reader-pane">
      <YandexMailReader selected={selected} message={detail} loading={detailLoading}
    ...
    {dialogMode && <YandexMailAccountDialog accounts={accounts}
