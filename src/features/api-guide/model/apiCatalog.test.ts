...
const routePattern = /(?:app|iCloudRoutes|gmailRoutes|microsoftRoutes|qqMailRoutes|naverMailRoutes|yandexMailRoutes|linuxDoMailRoutes|mailFeatureRoutes|outboundRateLimitRoutes|systemVersionRoutes)\.(get|post|put|patch|delete)\(\s*(?:['"]([^'"]+)['"]|(\w+))/g

// A few public routes (Microsoft Graph's push endpoints) register with an
// imported path constant rather than a literal, since the same constant also
// builds the callback URL sent to Microsoft. Resolve those from their export.
const constantFiles = [
  'email-worker/src/features/microsoft/microsoft-graph-notifications.ts',
]

function routeConstants(): Record<string, string> {
...
function sourceRouteKeys(): string[] {
  ...
  return routeFiles.flatMap((filename) => {
    ...
    return [...source.matchAll(routePattern)].map((match) => {
      const literal = match[2] ?? constants[match[3]]
      if (literal === undefined) {
        throw new Error(`Unresolved route path constant "${match[3]}" in ${filename}`)
      }
      const path = literal.startsWith('/api/') ? literal : `/api${literal}`
...
describe('API catalog', () => {
  it('documents every Worker HTTP route exactly once', () => {
    ...
    expect(documented).toHaveLength(171)
  })
