...
export async function parseGmailMetadata(
  fetchLine: string,
  headers: Uint8Array,
): Promise<GmailMessageMetadata> {
  const uid = Number(numericAttribute(fetchLine, 'UID'))
  const gmailMessageId = numericAttribute(fetchLine, 'X-GM-MSGID')
  if (!Number.isSafeInteger(uid) || uid < 1 || !gmailMessageId) {
    throw new Error('Gmail FETCH 响应缺少稳定消息标识。')
  }
  ...
  const labels = attributeList(fetchLine, 'X-GM-LABELS')
  const contentType = parsed.headers.find(({ key }) => key === 'content-type')?.value || ''
  return {
    gmailMessageId,
    gmailThreadId: numericAttribute(fetchLine, 'X-GM-THRID'),
    imapUid: uid,
    ...
    hasAttachments: /multipart\/mixed/i.test(contentType)
      || /"ATTACHMENT"/i.test(fetchLine),
  }
}
...
export async function parseGmailMessage(
  data: Uint8Array,
  uid: string,
): Promise<{ message: GmailMessageDetail; parsedAttachments: Attachment[] }> {
...
export function gmailAttachmentContent(
  attachments: Attachment[],
  partId: string,
): { metadata: GmailAttachment; data: Uint8Array } | null {
