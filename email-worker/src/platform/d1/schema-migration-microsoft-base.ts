export const MICROSOFT_MIGRATION = '0027_microsoft_imap.sql'
export const MICROSOFT_COMBINATION_PASSWORD_MIGRATION =
  '0028_microsoft_oauth_combination_password.sql'

/**
 * Base Microsoft IMAP tables (0027).
 *
 * Extracted from schema-migrations.ts to keep that file under the 600-line
 * limit, following the one-module-per-provider pattern already used by
 * schema-migration-yandex / -qq-identities / -external-mail.
 *
 * Note the messages table here is the ORIGINAL IMAP-only shape; 0036
 * (schema-migration-microsoft-transport.ts) rebuilds it with the transport
 * dimension. Both must stay in the list, in order.
 */
export const MICROSOFT_BASE_RECOVERY = {
...
/** Removal of password auth for Microsoft accounts (0028). */
export const MICROSOFT_COMBINATION_PASSWORD_RECOVERY = {
