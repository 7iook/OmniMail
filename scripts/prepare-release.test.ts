...
describe('release metadata preparation', () => {
  it('validates the matching versioned release notes file', () => {
    ...
    expect(result.stdout).toContain('v0.3.6.md')
    const notes = readFileSync(
      join(process.cwd(), 'docs', 'releases', 'web', 'v0.3.6.md'),
      'utf8',
    )
    expect(notes).toContain('### 新增')
    expect(notes).toContain('IMAP 已读状态同步')
    expect(notes).toContain('0022_consistency_guards.sql')
    expect(notes).toContain('无需新增 Worker 变量')
  ...
  it('rejects a tag that does not match package metadata', () => {
    ...
    expect(result.stderr).toContain('does not match tag v0.3.7')
  })
})
