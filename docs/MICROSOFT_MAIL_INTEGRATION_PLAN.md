...
- 后续需求已开放打开正文后精确写入 `\Seen`；仍不包含发信、删除、移动、归档、星标或其他 flags 写入；
...
使用 `BODY.PEEK[]`，避免正文取得前提前改变远端状态；正文成功后仅允许对已校验 UID 执行
`UID STORE ... +FLAGS.SILENT (\Seen)`。继续沿用现有单封、单附件、HTML 沙箱和文件名安全限制。
正文和附件不长期保存在 D1/R2。
...
10 分钟窗口允许 50 次凭据验证，以容纳两批完整的 25 项导入；前端逐项提交并展示真实进度，避免
单次长请求无反馈，也不能让一个合法批次在处理中途触发自身限速。
...
- `BODY.PEEK[]` 本身不主动标记已读，正文成功后精确写入 `\Seen`，失败不阻断正文响应；
...
- 发信、回复、删除、移动、归档、星标和除 `\Seen` 外的远端 flags 写入；
