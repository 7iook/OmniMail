...
- 状态：只读 Web MVP 已部署；实际 Cloudflare Worker 登录、收件列表和正文读取验收通过
- 记录日期：2026-08-22
- 当前变更范围：阶段 0 协议验证与阶段 1 只读 Web MVP；不包含 SMTP 和多端支持
...
只读 MVP 已完成阶段 0 验证。扩展到 SMTP 发信前仍需准备可撤销测试令牌和明确测试授权，
重新确认当时的服务规则与发信额度，并单独完成 SMTP 行为验证。
