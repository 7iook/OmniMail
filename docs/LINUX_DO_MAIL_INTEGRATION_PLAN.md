...
- 状态：只读 Web MVP 已部署；SMTP 发件 MVP 已实现，等待生产迁移与真实自发自收验证
- 记录日期：2026-08-22
...
当前已重新确认 SMTP 465、认证令牌建议及 `50/day` 公布额度。部署阶段 2 前仍需执行迁移
`0024_linux_do_mail_outbound.sql`，并在用户确认测试收件地址后，从实际 Worker 完成一封
自发自收验证；验证前不启用附件或向服务器 Sent 文件夹追加副本。
