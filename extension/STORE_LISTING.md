...
在任意网页生成和填入 OmniMail 或 iCloud 地址，并查看 OmniMail、iCloud、Gmail 与 QQ 来信。
...
- 自动显示用户已在 Web 连接的 Gmail 与 QQ 邮箱，不在扩展中收集第三方凭据。
- 在 OmniMail、iCloud、Gmail 与 QQ 来源之间切换，筛选账号、搜索邮件，并查看经过
  安全处理的正文。
...
- `alarms`：每分钟检查用户 OmniMail 收件箱，以更新未读徽标并发现新邮件。
- `identity`：使用 Chrome Identity 回调、PKCE 和一次性授权码完成用户明确确认的登录。
- `notifications`：在检测到新的 OmniMail 邮件时显示浏览器通知。
- `storage`：在本机保存用户选择的站点地址、主题、悬浮窗口设置、新邮件判断标识和
  受保护的扩展登录状态。
...
- `content_scripts.matches` 的普通 HTTP/HTTPS 网页访问：显示 Float 悬浮入口，并在
  用户点击“填入网页”后定位当前页面的邮箱输入框。扩展不读取或上传浏览历史、网页
